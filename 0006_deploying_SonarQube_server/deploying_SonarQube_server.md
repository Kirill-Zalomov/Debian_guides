...
7. [Проведение статического анализа кодовой базы проекта](#сhapter_7)  
...
<a name="сhapter_7"></a>
### 7) Проведение статического анализа кодовой базы проекта  

В этой части будем использовать следующие рекомендации: [docs.sonarsource.com](https://docs.sonarsource.com/sonarqube-server/10.8/analyzing-source-code/overview/).  

Создадим локальный проект для статического анализа (рисунок 20):  

![Создание локального проекта для статического анализа](images/20.png)  
Рисунок 20 --- Создание локального проекта для статического анализа  

После нажатия "Create a local project" откроется окно, где нужно ввести название проекта и название главной ветки проекта в системе контроля версий (рисунок 21):  

![Настройка локального проекта](images/21.png)  
Рисунок 21 --- Настройка локального проекта  

Введём следующие данные, соответствующие выбранному проекту для тестирования работы статического анализатора (рисунок 22).  

![Настройка локального проекта](images/22.png)  
Рисунок 22 --- Настройка локального проекта  

Название главной ветки в системе контроля версий, скорее всего, будет либо "main", либо "master" (рисунок 23).  

![Определение главной ветки в системе контроля версий проекта](images/23.png)  
Рисунок 23 --- Определение главной ветки в системе контроля версий проекта  

Определим правило, по которому будет определяться, какой код считается новым (рисунок 24):  

![Настройка локального проекта](images/24.png)  
Рисунок 24 --- Настройка локального проекта  

Затем отметим, что собираемся проводить статический анализ локально (рисунок 25), поскольку в статье не настраивались инструменты для CI/CD.  

![Настройка локального проведения статического анализа](images/25.png)  
Рисунок 25 --- Настройка локального проведения статического анализа  

Сгенерируем токен проекта, который необходим для локального проведения статического анализа (рисунок 26-27):  

![Генерация токена проекта для локального проведения статического анализа](images/26.png)  
Рисунок 26 --- Генерация токена проекта для локального проведения статического анализа  

![Генерация токена проекта для локального проведения статического анализа](images/27.png)  
Рисунок 27 --- Генерация токена проекта для локального проведения статического анализа  

Выберем систему сборки, которая используется в проекте (рисунок 28):  

![Выбор системы сборки проекта](images/28.png)  
Рисунок 28 --- Выбор системы сборки проекта  

На то, что в проекте используется система сборки Maven указывает файл pom.xml в папке проекта (рисунок 29):  

![Файл, указывающий на использование системы сборки Maven](images/29.png)  
Рисунок 29 --- Файл, указывающий на использование системы сборки Maven  

После выбора системы сборки, SonarQube предоставит команду, которую необходимо запустить в терминале для локального проведения статического анализа кода проекта (рисунок 30):  

![Команда, которую необходимо запустить в терминале для локального проведения статического анализа кода проекта](images/30.png)  
Рисунок 30 --- Команда, которую необходимо запустить в терминале для локального проведения статического анализа кода проекта  

Запустим терминал, перейдём в папку проекта и запустим полученную ранее команду (рисунок 31):  

![Команда, которую необходимо запустить в терминале для локального проведения статического анализа кода проекта](images/31.png)  
Рисунок 31 --- Команда, которую необходимо запустить в терминале для локального проведения статического анализа кода проекта  

В зависимости от размера кодовой базы проекта статический анализ может выполняться от одной до нескольких минут. После завершения проведения статического анализа в терминале появится сообщение, приведённое на рисунке 32:  

![Завершения локального проведения статического анализа](images/32.png)  
Рисунок 32 --- Завершения локального проведения статического анализа  

В web-интерфейсе севера SonarQube после завершения статического тестирования страница автоматически обновится и покажет краткие результаты анализа (рисунок 33):  

![Краткие результаты статического анализа проекта](images/33.png)  
Рисунок 33 --- Краткие результаты статического анализа проекта  

После первого проведения статического анализа в web-интерфейсе станут доступны несколько разделов, включая "Issues" (рисунок 34) и "Security HotSpot" (рисунок 35).  

![Вкладка "Issues" в web-интерфейсе SonarQube](images/34.png)  
Рисунок 34 --- Вкладка "Issues" в web-интерфейсе SonarQube  

![Вкладка "Security HotSpot" в web-интерфейсе SonarQube](images/35.png)  
Рисунок 35 --- Вкладка "Security HotSpot" в web-интерфейсе SonarQube  

Таким образом, в ходе статьи были достигнуты все поставленные [цели](#сhapter_1).
